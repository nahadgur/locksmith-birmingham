// components/ServicePageClient.tsx
'use client';

import { useState, useMemo } from 'react';
import Link from 'next/link';
import { MapPin, Clock, Shield, Star, Search, CheckCircle, ArrowRight, ChevronDown, Award, Users } from 'lucide-react';
...
export function ServicePageClient({ service }: Props) {
  ...
  const content = serviceContent[service.id] || serviceContent['emergency-lockout'];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mt-6">
              <div>
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold tracking-tight mb-6">
                  {service.title} Birmingham | {siteConfig.name}
                </h1>
                <p className="text-xl text-gray-300 mb-8 leading-relaxed">
                  {siteConfig.name} connects you with vetted {service.title.toLowerCase()} specialists across Birmingham. {service.description}
                </p>
                <div className="space-y-3">
                  {['Compare up to 3 free quotes', 'Every locksmith vetted and insured', `${totalCities}+ locations covered`].map((item, i) => (
            ...
            <div className="lg:col-span-2">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {content.benefits.map((b, i) => (
                    <div key={i} className="flex gap-4 p-5 bg-gray-50 rounded-xl border border-gray-100">
                      <div className="bg-brand-100 p-2 rounded-lg text-brand-600 flex-shrink-0 h-fit"><CheckCircle className="w-5 h-5" /></div>
                      <div>
              ...
              <section className="mb-14">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
                  <div>
                    <h2 className="text-2xl md:text-3xl font-display font-bold text-gray-900 mb-2">Find {service.title} Locksmiths</h2>
                    <p className="text-gray-600">Specialists for {service.title.toLowerCase()} covering {totalCities} towns and areas.</p>
                  </div>
                  <button onClick={() => setShowLocations(!showLocations)} className="flex items-center gap-2 text-brand-600 font-bold text-sm hover:underline self-start md:self-auto whitespace-nowrap">
                ...
                <div className="mb-6 max-w-md">
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Search your town or area..." value={searchQuery}
                ...
                <div className={`transition-all duration-500 overflow-hidden ${showLocations ? 'max-h-[9999px] opacity-100' : 'max-h-0 opacity-0'}`} aria-hidden={!showLocations}>
                ...
                {!showLocations && (
                  <p className="text-sm text-gray-500">
                    Search for your area above or <button onClick={() => setShowLocations(true)} className="text-brand-600 font-medium hover:underline">browse all locations</button> to find {service.title.toLowerCase()} locksmiths near you.
                  </p>
                )}
              </section>

              <section className="mb-14">
                <h2 className="text-2xl font-display font-bold text-gray-900 mb-4">Is {service.title} Right for Your Property?</h2>
                <p className="text-gray-600 mb-4">{content.candidateIntro}</p>
                <div className="bg-brand-50 rounded-xl p-6 border border-brand-100">
                  <ul className="space-y-3">
                    {content.candidates.map((c, i) => (
                      <li key={i} className="flex items-start gap-3 text-gray-700">
                        <CheckCircle className="w-4 h-4 text-brand-500 flex-shrink-0 mt-0.5" /><span className="text-sm">{c}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <section className="mb-14">
                <h2 className="text-2xl font-display font-bold text-gray-900 mb-6">The Process</h2>
              ...
              {/* Related Guides - Query Network Links */}
              ...
              <div className="mb-14"><FAQ faqs={combinedFaqs} title={`${service.title} FAQs`} /></div>
              <section className="mb-14">
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-28 space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                  ...
                  <p className="text-gray-600 mb-5 text-sm">Free, no-obligation match with vetted locksmiths in your area.</p>
                  <button onClick={() => setIsModalOpen(true)} className="block w-full btn-primary text-center">Find a Locksmith</button>
                  <div className="mt-5 pt-5 border-t border-gray-100 space-y-3">
                    {[
                      { icon: <Clock className="w-4 h-4 text-brand-500" />, text: "Available 24/7 for emergencies" },
                      { icon: <Shield className="w-4 h-4 text-brand-500" />, text: "50+ jobs per locksmith" },
