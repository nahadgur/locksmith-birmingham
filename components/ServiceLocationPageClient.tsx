// components/ServiceLocationPageClient.tsx
'use client';
...
import { guides, getRelatedGuides } from '@/data/guides';
...
export function ServiceLocationPageClient({ service, cityName, locationSlug }: Props) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const content = serviceLocationContent[service.id] || serviceLocationContent['emergency-lockout'];
  const intro = content.intro(cityName);
  const steps = content.steps(cityName);
  const whyPoints = content.whyPoints(cityName);
  const micro = getMicroSemantics(service.id, cityName);
  const relatedGuides = getRelatedGuides(service.id);

  const benefits = [
    { icon: <Award className="w-6 h-6" />, title: 'Minimum 50 Residential Jobs', desc: `Every ${cityName} locksmith in our network specialises in this work and has a verified job history before receiving a single referral from us.` },
    { icon: <Clock className="w-6 h-6" />, title: 'Fast Response Guaranteed', desc: `Most locksmiths covering ${cityName} can respond within 30-60 minutes for emergencies, with flexible scheduling for non-urgent work.` },
    { icon: <Shield className="w-6 h-6" />, title: 'Insured and Guaranteed', desc: `Public liability cover and written guarantees on both parts and labour are required from every locksmith before we refer any ${cityName} enquiries.` },
    { icon: <Users className="w-6 h-6" />, title: 'Matched to Your Job Type', desc: `We connect you with ${cityName} locksmiths who have specific experience with ${service.title.toLowerCase()}, not a general list of whoever is available.` },
  ];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mt-6">
              <div>
                <div className="inline-flex items-center gap-2 bg-brand-500/20 text-brand-300 px-3 py-1 rounded-full text-sm font-medium mb-6 border border-brand-500/30">
                  <MapPin className="w-4 h-4" /> Vetted Locksmiths in {cityName}
                </div>
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold tracking-tight mb-6">
                  {service.title} in <span className="text-brand-400">{cityName}</span> | {siteConfig.name}
                </h1>
                <p className="text-xl text-gray-300 mb-8 leading-relaxed">
                  {siteConfig.name} connects you with vetted {service.title.toLowerCase()} specialists covering {cityName}. Free assessment, written quotes, no obligation to proceed.
                </p>
                <div className="space-y-4 mb-8">
                  {[
                    `${service.title} specialists in ${cityName}, verified and active`,
                    'Up to three independent quotes, each with a free assessment',
                    'Insured, guaranteed, and working to British Standards',
                  ].map((item, i) => (
                ...
                <div className="flex items-center gap-4 text-sm text-gray-400">
                  ...
                  <span>Highly rated by homeowners across Birmingham</span>
                </div>
              </div>
              <div>
        ...
        <div className="container-width py-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              ...
              <section className="mb-12">
                <h3 className="text-2xl font-display font-bold text-gray-900 mb-4">Why Get {service.title} in {cityName} Through {siteConfig.name}?</h3>
              ...
              {/* Related Guides - Internal Links to Query Network */}
              {relatedGuides.length > 0 && (
                <section className="mb-12">
                  <h2 className="text-2xl font-display font-bold text-gray-900 mb-4">Locksmith Guides for {cityName} Homeowners</h2>
              ...
              <div className="sticky top-28 space-y-8">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                  ...
                  <ul className="space-y-2">
                    {Object.values(LOCATIONS).flat().filter(c => c !== cityName).slice(0, 5).map(city => {
                ...
                <div className="bg-brand-900 text-white p-6 rounded-2xl shadow-lg">
                  ...
                  <p className="text-brand-100 text-sm mb-4">Get matched with a vetted {cityName} locksmith. Free quotes, no obligation, available 24/7 for emergencies.</p>
          ...
          <div className="bg-brand-900 rounded-2xl p-8 md:p-12 text-center mt-12">
            ...
            <p className="text-brand-200 mb-8 max-w-2xl mx-auto">Submit your enquiry in under two minutes. {siteConfig.name} will match you with up to three vetted {cityName} locksmiths for free assessments, written quotes, and no obligation at any stage.</p>
