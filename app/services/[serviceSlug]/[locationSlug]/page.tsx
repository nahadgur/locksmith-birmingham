...
export default function ServiceLocationPage({ params }: { params: { serviceSlug: string; locationSlug: string } }) {
  ...
  const benefits = [
    { icon: <Award className="w-6 h-6" />, title: 'Minimum 50 Residential Installs', desc: `Every ${cityName} installer in our network specialises in this work and has a verified project history before receiving a single referral from us.` },
    ...
    { icon: <Shield className="w-6 h-6" />, title: 'Insured and Warranted', desc: `Public liability cover and written warranties on both the product and the installation are required from every installer before we refer any ${cityName} enquiries.` },
    { icon: <Users className="w-6 h-6" />, title: 'Matched to Your Project Type', desc: `We connect you with ${cityName} installers who have specific experience with ${service.title.toLowerCase()}, not a general list of whoever is available.` },
  ];
  ...
  const localBusinessSchema = {
    ...
    description: `Find vetted ${service.title.toLowerCase()} specialists in ${cityName}. Free site survey, written quotes, no obligation.`,
    areaServed: {
      '@type': 'City',
      name: cityName,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mt-6">
              <div>
                ...
                <p className="text-xl text-gray-300 mb-8 leading-relaxed">
                  {service.title.toLowerCase()} specialists covering {cityName}. Site survey at no charge, written quotes, no obligation to proceed.
                </p>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-400">
                  ...
                  <span>Highly rated by homeowners</span>
                </div>
              </div>
              <div>
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-28 space-y-8">
                ...
                <div className="bg-brand-900 text-white p-6 rounded-2xl shadow-lg">
                  ...
                  <p className="text-brand-100 text-sm mb-4">0% finance available at most {cityName} installers. Spread the cost over 6 to 36 months with nothing to pay upfront.</p>
